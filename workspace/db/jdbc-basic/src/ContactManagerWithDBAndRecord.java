...
public class ContactManagerWithDBAndRecord {

	private java.util.Scanner scanner = new java.util.Scanner(System.in);

	private ContactDaoWithRecord dao = new ContactDaoWithRecord();
	
	public ContactManagerWithDBAndRecord() {}
	...
	public void manage() {
		...
		program : while (true) {
			...
			case "1": // 연락처 등록 
				// 1. 연락처 입력 + 인스턴스 생성
				ContactRecord contact = inputNewContact();
				// 2. 인스턴스를 연락처 테이블에 저장
			...
			case "3": // 삭제				
				...
				// 1-2. 검색 수행
				ArrayList<ContactRecord> contactsToDelete = dao.selectContactsByName(nameToDelete);
				// 1-3. 검색결과 출력
				if (contactsToDelete.size() == 0) {
					System.out.println("삭제 대상 연락처가 없습니다.");
				} else {
					System.out.println("[ 검색된 연락처 목록 ]");
					for (ContactRecord c: contactsToDelete) {
			...
			case "4": // 검색 (이름기준) 
				...
				ArrayList<ContactRecord> searchResult = dao.selectContactsByName(nameToSearch);
				...
				if (searchResult.size() == 0) {
					...
					for (ContactRecord c: searchResult) {
						System.out.println(c);
					}
				}
			...
			case "5": // 연락처 목록
				...
				ArrayList<ContactRecord> contacts = dao.selectAllContacts();
				// 2. 조회 결과 출력
				if (contacts.size() == 0) {
	...
	public ContactRecord inputNewContact() {
		// 1. 이름, 이메일, 전화번호 사용자입력 -> 입력된 내용을 인스턴스에 저장
		System.out.print("이름 : ");
		...
		// 2. 연락처 인스턴스 만들기 ( 1개의 연락처 정보 )
		// ContactRecord contact = new ContactRecord(-1, name, phone, email, null);
		ContactRecord contact = new ContactRecord(name, phone, email);
	...
	public static void main(String[] args) {
		...
		ContactManagerWithDBAndRecord contactManager = new ContactManagerWithDBAndRecord();
